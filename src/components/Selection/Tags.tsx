...
const Tags = ({ tagHandler }: tagsProps) => {
    ...
    const filterTags = (e: React.ChangeEvent<HTMLInputElement>) => {
        setFilter(tags.split(",").filter(tag => tag.toLowerCase().includes(e.target.value.toLowerCase())))
    }

    return (
        <div className={`flex items-start gap-5 min-w-full p-10 pt-0 flex-col sm:flex-row-reverse animate-animate-fade-in`}>
            <input onChange={filterTags} type="text" placeholder="Search Genre" className="input input-bordered input-secondary w-full max-w-xs mt-2" />
                        ...
                        filter.sort().map(tag => {
                            return (
                                <label key={tag} className="cursor-pointer label hover:bg-base-100 rounded p-3">
                                    ...
                                    <input name={tag} onChange={tagHandler} value={tag.trim().toLowerCase()} type="checkbox" className="checkbox checkbox-accent" />
                                </label>
                            )
                        }
                        )
